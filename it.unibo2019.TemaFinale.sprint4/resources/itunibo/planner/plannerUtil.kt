...
//        val y1     = initialState!!.y
	...
	//MIO
	fun saveMap(  map : String, fname : String){		
        println("saveMap in $fname")
		val newMap = map.replace("@","\n")
		val pw = PrintWriter( FileWriter(fname+".txt") )
		pw.print( newMap )
		pw.close()
		
		UtilMap.getUtilMap().updateMap(map)
 ...
 */
	...
	fun setGoal( x: Int, y: Int, direction : String ) {
        try {
			var dir     = RobotState.Direction.DOWN  //init
			when( direction ){
            ...
            //println("setGoal $x,$y, $direction while robot in cell: ${getPosX()}, ${getPosY()} direction=${getDirection()}")	
            RoomMap.getRoomMap().put(x, y, Box(false, true, false))
			//initialState = RobotState(getPosX(), getPosY(), initialState!!.direction ) 
            goalTest = GoalTest { state  : Any ->
                val robotState = state as RobotState
				(robotState.x == x && robotState.y == y && robotState.getDirection() == dir)
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }
